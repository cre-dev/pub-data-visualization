...
def heatmap(x, y,
     ...
     if 'x_order' in kwargs:
         ...
         x_names = [t for t in list(set([v for v in x]))]
     x_to_num = {p[1]:p[0] for p in enumerate(x_names)}

     if 'y_order' in kwargs:
         y_names = [t for t in kwargs['y_order']]
     else:
         y_names = [t for t in list(set([v for v in y]))]
     ...
     kwargs_pass_on = {k:v for k,v in kwargs.items() if k not in [ 'color', 'palette', 'color_range', 'size', 'size_range', 'size_scale', 'marker', 'x_order', 'y_order', 'invert_yaxis', 'show_grid', ]}
